...
def calc_fractal_opencl(chunks, maxiter):
    ...
    for chunk_input in chunks:
        ...
        chunk_input_on_device = cl.Buffer(ctx,
                                          mf.READ_ONLY | mf.COPY_HOST_PTR,
                                          hostbuf=chunk_input)

        chunk_output_on_device = cl.Buffer(ctx, mf.WRITE_ONLY,
                                           int(chunk_input.nbytes / 4))
        # divided by 4 because our inputs are 64 bits but outputs are 16 bits

        # Call the kernel on this chunk
        prg.mandelbrot(queue, chunk_shape, None, chunk_input_on_device,
                       chunk_output_on_device, np.uint16(maxiter))
...
def apply_blur_opencl(image, radius, strength):
    ...
    #define STRENGTH {}
    """.format(image_dims[0], image_dims[1], int(radius), int(radius / 2),
               int(strength))
    ...
    image_on_device = cl.Buffer(ctx,
                                mf.READ_ONLY | mf.COPY_HOST_PTR,
                                hostbuf=image)
    filter_on_device = cl.Buffer(ctx,
                                 mf.READ_ONLY | mf.COPY_HOST_PTR,
                                 hostbuf=filt)
    output_on_device = cl.Buffer(ctx, mf.WRITE_ONLY, image.nbytes)
    output = np.zeros(image_dims, dtype=np.float)

    prg.convolve(queue, image_dims, None, image_on_device, output_on_device,
                 filter_on_device)
...
if __name__ == '__main__':
    ...
    class Mandelbrot(object):
        def __init__(self):
            ...
            self.render((self.centre_x - self.w_range) / self.zoom,
                        (self.centre_x + self.w_range) / self.zoom,
                        (self.centre_y - self.h_range) / self.zoom,
                        (self.centre_y + self.h_range) / self.zoom,
                        self.iterations)
            self.save_image()
        ...
        def save_image(self):
            ...
            if (self.beautify):
                filtered = normalized
                b_filtered = ndimage.uniform_filter(filtered,
                                                    size=int(11 *
                                                             self.b_spread))
                g_filtered = ndimage.uniform_filter(filtered,
                                                    size=int(11 *
                                                             self.g_spread))
                r_filtered = ndimage.uniform_filter(filtered,
                                                    size=int(11 *
                                                             self.r_spread))
            ...
            cv2.imwrite(
                self.fname,
                cv2.merge(
                    (np.rint(b).astype(np.uint8), np.rint(g).astype(np.uint8),
